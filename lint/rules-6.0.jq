# lint/rules-6.0.jq — Zabbix 6.0 API JSON Schema convention rules.
#
# Sibling of lint/rules-7.0.jq (7.0). Identical checks; only the version-bearing
# literals differ ($id host path and $comment Source URL cite 6.0, not 7.0).
#
# Input : one schema document (a single schemas/6.0/<object>/<object>.<method>.json).
# Args  : --arg object <object>   --arg method <method>
# Output: one line per convention violation. No output means the file is clean.
#
# Reused by lint-6.0.sh.
...
[
  ...
  want(.["$id"] == "https://zabbix.com/6.0/api/\($object)/\($object).\($method)";
       "$id must be https://zabbix.com/6.0/api/\($object)/\($object).\($method) (got \(.["$id"] // "null"))"),
  ...
  want((.["$comment"] // "") | test("Source: https://www\\.zabbix\\.com/documentation/6\\.0/en/manual/api/reference/");
